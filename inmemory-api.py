...
con.execute("create table pessoas(id int, nome string)")

for row in csv_file:
    pid += 1
    con.execute("insert into pessoas(id, nome) values (?, ?)", (int(pid), str(row)),)
#--------------------
    lin['id']   = pid
    lin['nome'] = row
